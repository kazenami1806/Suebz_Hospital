...
<html lang="en">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <div class="card-body">
                            <table id="gambar" class="table table-bordered hover">
                                <button type="button" class="btn btn-success" style="margin-bottom: 20px;" onclick="window.location.href = '../media/proses/add_gambar.php'" >Tambah Gambar</button>
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>Name Image</th>
                                ...
                                <?php
                                    ...
                                    while ($row = mysqli_fetch_assoc($data)) {
                                        echo "<tr>
                                                ...
                                                <td>" . $row['name_img'] . "</td>
                                                <td>" . $row['notes'] . "</td>
                                                <td><img src='../../img/slideshow/" . $row['img'] . "' width=25%></td>
                                                <td>
                                                    <button class='btn btn-primary' onclick=\"location.href='../media/proses/edit_gambar.php?id_gambar=" . $row['id_gambar'] . "'\">Edit</button>
                                                    <button class='btn btn-danger' onclick=\"confirmDelete(" . $row['id_gambar'] . ")\">Delete</button>
                                                </td>
                                            </tr>";
                                        $no++;
                                    }
    ...
    function confirmDelete(id_gambar) {
        if (confirm("Apakah Anda yakin ingin menghapus gambar ini?")) {
            // If user confirms, redirect to delete image script
            redirectToDelete(id_gambar);
        }
    }

    // Function to redirect to delete image script
    function redirectToDelete(id_gambar) {
        window.location.href = "../media/proses/m_delete_gambar.php?id_gambar=" + id_gambar;
    }
    ...
    function initDataTable() {
        new DataTable('#gambar', {
            columnDefs: [
                { className: 'dt-center', width: 20, targets: 0 },
                { className: 'dt-center', width: 200, targets: 4 },
                { className: 'dt-center', width: 700, targets: 3 }
            ],
        });
    }

    // Call initDataTable() when the document is ready
